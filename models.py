...
class Blackbody(Fittable1DModel):
    ...
    Free parameters include normalization, and temperature.

    Equation:
    S_grey = norm*B(nu,tdust)

    Parameters:
        norm: normalization of the blackbody
        temp: temperature in Kelvin
        lumD: luminosity distance in Mpc
        B(nu,tdust): is the standard blackbody equation
    """
    
    norm = Parameter(default=-20.0, bounds=(-100, 100))
    temp = Parameter(default=5000.0, bounds=(1000., 6000.))

    def __init__(self, norm, temp, lumD=None, redshift=0.001):

        self.set_redshift(redshift)
        self.set_lumD(lumD)

        super(Blackbody, self).__init__(norm, temp)

    def evaluate(self, x, norm, temp):

        n = c_micron/x
        bnu = ((2 * planck_h * n**3 / c**2) *
               1 / (np.exp(planck_h * n / k_b / temp) - 1))
        return 10**norm * bnu * 1e23
...
class GreyPowBlackbody(Fittable1DModel):
    """
    Single temperature blackbody.
    Free parameters include normalization, and temperature.
    ...
    norm = Parameter(default=-20.0, bounds=(-100, 100))
    temp = Parameter(default=5.0, bounds=(0.5, 10.0))
    mdust = Parameter(bounds=(1, 10))
    tdust = Parameter(bounds=(0, 100))
    beta = Parameter(default=2.0, bounds=(0, 5))
    pownorm = Parameter(bounds=(-10, 10))
    alpha = Parameter(default=2.0, bounds=(1, 2))
    wturn = Parameter(default=40.0, bounds=(10, 100))

    def __init__(self, norm, temp, mdust, tdust, beta, pownorm, alpha, wturn,
                 kappa0=0.192, lambda_norm=350., lumD=None, redshift=0.001):
        ...
        self.n_components = 3
        self.comp_names = ['Greybody', 'Exp. Powerlaw', 'Blackbody']

        super(GreyPowBlackbody, self).__init__(norm, temp, mdust, tdust,
                                               beta, pownorm, alpha, wturn)

    def evaluate(self, x, norm, temp, mdust, tdust, beta, pownorm, alpha, wturn):
        
        n = c_micron/x
        md = (10**mdust)*Msun_to_g
        bnorm = (2 * planck_h * n**3 / c**2)
        bgrey = (bnorm * 1 / (np.exp(planck_h * n / k_b / tdust) - 1))
        ...
        flux_bb =10**norm * (bnorm * 1 / (np.exp(planck_h * n / k_b / (temp*1000)) - 1))*1e23
        return flux_grey + flux_plaw + flux_bb
    ...
    def eval_bb(self, x):
        """
        Method to calculate only the blackbody component flux.
        ...
        return 10**self.norm * (bnorm * 1 / (np.exp(planck_h * n / k_b / (self.temp*1000)) - 1))*1e23
        
    def eval_comps(self, x):

        comp1 = self.eval_grey(x)
        comp2 = self.eval_plaw(x)
        comp3 = self.eval_bb(x)
    ...
    def calc_grey_lum(self, lower=8.0, upper=1000.0):
    ...
    def calc_bb_lum(self, lower=8.0, upper=1000.0):
        ...
        flux_density = self.eval_bb(waves)/1e23
        integral = -np.trapz(flux_density, freq)
        lum = (4*np.pi)*(self.lumD_cm)**2*integral/const.L_sun.cgs.value
...
class Dale2014(object):
    ...
    def calc_luminosity(self, alph, fa, norm=0.0, low=8., high=1000.):
        ...
        waves = self.waves[(self.waves <= high) & (self.waves >= low)]
        sed = sed[(self.waves <= high) & (self.waves >= low)]
        freq = c_micron/waves
...
class DecompIR(object):
    ...
    def __init__(self, redshift=0.001, lumD=None):
        ...
        sy16_model = np.loadtxt(direct+'/DecompIR/sy16_template.txt')
